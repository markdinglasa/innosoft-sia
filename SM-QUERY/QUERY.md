...
END AS [Refund Amount],
    ...
    MAX(
        CASE
            ...
            ELSE    '0.00'
        END
    ) AS [Senior Citizen Discount Amount],
    MAX(
        CASE
    ...
    MAX(
        CASE
            ...
            ELSE    '0.00'
        END
    ) AS [PWD Discount Amount],
    MAX(
        CASE
            WHEN    ([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL) AND ([MstDiscount].[Discount] = 'National Athlete Representative')
            THEN    COALESCE(CONVERT(VARCHAR, (([TotalDiscount].[TotalDiscountAmount])), 1), 0.00)
            ELSE    '0.00'
        END
    ) AS [National Coach / Athlete / Medal of Valor Discount Amount],
