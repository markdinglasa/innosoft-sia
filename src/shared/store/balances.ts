...
import { SYSTEM_BALANCES } from '@shared/constants'
import { Balances, IpcChannel } from '@shared/types'
...
const balances = createSlice({
  ...
  reducers: {
    ...
    setBalance: (
      state: Balances,
      { payload }: PayloadAction<{ balance: number; networkId: string }>
    ) => {
...
export const { _deleteBalance, _initializeBalance, initializeBalances, setBalance, setBalances } =
  balances.actions
