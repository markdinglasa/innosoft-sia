...
async function generateOldXml(
  ...
  const salesQ = `
    ...
    ${PreviousNoTaxSale} + SUM(ROUND(CASE WHEN(((ISNULL([TrnCollection].[IsReturn], 0) =  0 AND ISNULL([TrnSales].[IsCancelled],0) = 0)) AND (ISNULL([TrnSalesLine].[TaxAmount],0)<=0)) THEN [TrnSalesLine].[Amount] ELSE 0 END, 2)) AS [newnotaxsale],
