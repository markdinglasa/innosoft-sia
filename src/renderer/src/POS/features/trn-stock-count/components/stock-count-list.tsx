import { Checklist as CountIcon, Delete as DeleteIcon, Edit as EditIcon } from '@mui/icons-material'
import {
  Box,
  Button,
  CircularProgress,
  ...
  return (
    <>
        ...
        <Table stickyHeader size="small">
          ...
          <TableBody>
            {items.map((item: any) => (
              ...
              >
                <TableCell>
                  <CountIcon color="action" sx={{ fontSize: 25 }} />
                </TableCell>
                <TableCell>
                  <Typography variant="body2" fontWeight="medium">
                    {item.stockCountNumber || `SC-${item.id}`}
                  </Typography>
                </TableCell>
                <TableCell>
                  {item.stockCountDate ? new Date(item.stockCountDate).toLocaleDateString() : '—'}
                </TableCell>
      ...
      <Dialog open={deleteId !== null} onClose={() => setDeleteId(null)}>
        ...
        <DialogContent>
          <DialogContentText>Delete this stock count? This cannot be undone.</DialogContentText>
        </DialogContent>
        <DialogActions>
