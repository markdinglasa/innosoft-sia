import { Delete as DeleteIcon, PointOfSale as TerminalIcon } from '@mui/icons-material'
import {
  Button,
  Dialog,
  DialogActions,
...
export const TerminalList: React.FC = () => {
  ...
  const canDelete = hasPermission(SystemPermissions.TERMINAL_REMOVE)
  const canEdit = hasPermission(SystemPermissions.TERMINAL_EDIT)
    ...
    <>
      ...
      >
        <Table stickyHeader size="small">
          ...
          <TableBody>
            <TableSkeleton isLoading={isLoading} columns={4} rows={30}>
              {items.map((item: any) => (
                <TableRow
                  key={item.id}
                  hover
                  onClick={() => {
                    if (!canEdit) {
                      displayToast(
                        'You do not have permission to edit this terminal.',
                        ToastType.error
                      )
                      return
                    }
              ...
              {isError && (
                <TableRow>
                  <TableCell colSpan={6} align="center" sx={{ py: 4 }}>
                    <Typography color="error">Failed to load terminal(s).</Typography>
                  </TableCell>
                </TableRow>
              )}
