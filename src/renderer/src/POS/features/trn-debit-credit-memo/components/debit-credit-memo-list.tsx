...
export const DebitCreditMemoList: React.FC = () => {
  ...
  const handleDelete = async (id: number) => { if (window.confirm('Delete this memo?')) await deleteMutation.mutateAsync(id) }
  ...
  return (
    <TableContainer component={Paper} variant="outlined" sx={{ maxHeight: 'calc(100vh - 250px)' }}>
      <Table stickyHeader size="small"><TableHead><TableRow>
        <TableCell width={50}></TableCell><TableCell>Memo #</TableCell><TableCell>Date</TableCell>
        <TableCell>Particulars</TableCell><TableCell align="right">Actions</TableCell>
      </TableRow></TableHead>
        <TableBody>{items.map((item: any) => (
          <TableRow key={item.id} hover onClick={() => handleEdit(item.id)} sx={{ cursor: 'pointer' }}>
            <TableCell><MemoIcon color="action" fontSize="small" /></TableCell>
            <TableCell><Typography variant="body2" fontWeight="medium">{item.dcMemoNumber || `DCM-${item.id}`}</Typography></TableCell>
            <TableCell>{item.dcMemoDate ? new Date(item.dcMemoDate).toLocaleDateString() : '—'}</TableCell>
          ...
          {items.length === 0 && <TableRow><TableCell colSpan={5} align="center" sx={{ py: 4 }}><Typography variant="body2" color="text.secondary">No memos found.</Typography></TableCell></TableRow>}
