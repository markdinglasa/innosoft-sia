import { Delete as DeleteIcon, Edit as EditIcon, Percent as DiscountIcon } from '@mui/icons-material'
...
export const DiscountList: React.FC = () => {
  ...
  const handleDelete = async (id: number) => { if (window.confirm('Delete this discount?')) await deleteMutation.mutateAsync(id) }
  ...
  return (
    <TableContainer component={Paper} variant="outlined" sx={{ maxHeight: 'calc(100vh - 250px)' }}>
      <Table stickyHeader size="small"><TableHead><TableRow><TableCell width={50}></TableCell><TableCell>Name</TableCell><TableCell>Alias</TableCell><TableCell align="right">Rate (%)</TableCell><TableCell>VAT Exempt</TableCell><TableCell align="right">Actions</TableCell></TableRow></TableHead>
        <TableBody>{items.map((item: any) => (<TableRow key={item.id} hover onClick={() => handleEdit(item.id)} sx={{ cursor: 'pointer' }}><TableCell><DiscountIcon color="action" fontSize="small" /></TableCell><TableCell><Typography variant="body2" fontWeight="medium">{item.name}</Typography></TableCell><TableCell>{item.discountAlias || '—'}</TableCell><TableCell align="right">{Number(item.discountRate).toFixed(2)}</TableCell><TableCell><Chip label={item.isVATExempt ? 'Yes' : 'No'} size="small" color={item.isVATExempt ? 'warning' : 'default'} /></TableCell><TableCell align="right"><IconButton size="small" onClick={(e) => { e.stopPropagation(); handleEdit(item.id) }}><EditIcon fontSize="small" /></IconButton><IconButton size="small" color="error" onClick={(e) => { e.stopPropagation(); handleDelete(item.id) }}><DeleteIcon fontSize="small" /></IconButton></TableCell></TableRow>))}
          {items.length === 0 && <TableRow><TableCell colSpan={6} align="center" sx={{ py: 4 }}><Typography variant="body2" color="text.secondary">No discounts found.</Typography></TableCell></TableRow>}</TableBody></Table></TableContainer>)
}
