import { Delete as DeleteIcon, Edit as EditIcon, Inventory as StockIcon } from '@mui/icons-material'
...
export const StockInList: React.FC = () => {
  const { searchKeyword, setSelectedId, setIsFormOpen } = useStockInHubStore()
  const { useList, useDeleteMutation } = useMasterfile('stockIn')
  ...
  if (isError) return <Typography color="error">Failed to load stock-in records.</Typography>
  ...
  return (
    <TableContainer component={Paper} variant="outlined" sx={{ maxHeight: 'calc(100vh - 250px)' }}>
      <Table stickyHeader size="small"><TableHead><TableRow>
        <TableCell width={50}></TableCell><TableCell>Stock In #</TableCell><TableCell>Date</TableCell>
        ...
        <TableBody>{items.map((item: any) => (
          <TableRow key={item.id} hover onClick={() => handleEdit(item.id)} sx={{ cursor: 'pointer' }}>
            ...
            <TableCell><Typography variant="body2" fontWeight="medium">{item.stockInNumber || `SI-${item.id}`}</Typography></TableCell>
            <TableCell>{item.stockInDate ? new Date(item.stockInDate).toLocaleDateString() : '—'}</TableCell>
          ...
          {items.length === 0 && <TableRow><TableCell colSpan={5} align="center" sx={{ py: 4 }}><Typography variant="body2" color="text.secondary">No stock-in records found.</Typography></TableCell></TableRow>}
