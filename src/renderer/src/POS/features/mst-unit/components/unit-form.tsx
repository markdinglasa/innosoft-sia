...
import {
  Alert, Box, Button, CircularProgress, Divider, IconButton, TextField, Typography
} from '@mui/material'
...
interface UnitFormData { name: string; description: string }
...
export const UnitForm: React.FC = () => {
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<UnitFormData>({ defaultValues: { name: '', description: '' } })
      ...
      {isLoading ? <CircularProgress /> : (
        <form onSubmit={handleSubmit(onSubmit)} style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <Controller name="name" control={control} rules={{ required: 'Name is required' }}
            render={({ field }) => <TextField {...field} label="Unit Name" fullWidth margin="normal" error={!!errors.name} helperText={errors.name?.message} />} />
          <Controller name="description" control={control}
            render={({ field }) => <TextField {...field} label="Description" fullWidth margin="normal" multiline rows={3} />} />
          {saveMutation.isError && <Alert severity="error" sx={{ mt: 2 }}>Failed to save unit.</Alert>}
          <Box sx={{ mt: 'auto', pt: 3 }}>
            <Button type="submit" variant="contained" fullWidth startIcon={<SaveIcon />} disabled={saveMutation.isPending}>
