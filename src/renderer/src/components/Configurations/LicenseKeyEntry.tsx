...
export const LicenseKeyEntry = () => {
  ...
  const handleActivateLicenseKey = async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    //const response = await window.api.post('authenticate-licenseKey', key)
    if (!response.error) {
      ...
      <div className="w-full max-w-xl p-6 bg-white rounded-md shadow-md">
        ...
        <form className="space-y-4">
          ...
          <div className="flex justify-end px-2">
            ...
            <button className="shadow-md btn btn-primary rounded-md" onClick={handleActivateLicenseKey}>
              <span className='flex items-center justify-center'><FaKey className='mr-2'/>Activate</span>
            </button>
          </div>
        </form>
        {
